package policy

import "time"

// Limit3 队列/租约限额。
type Limit3 struct {
	Name       string
	MaxDepth   int
	LeaseTTL   time.Duration
	MaxRetries int
}

func DefaultLimit3() Limit3 {
	return Limit3{
		Name:       "limit3",
		MaxDepth:   1000 + 3,
		LeaseTTL:   time.Duration(3) * time.Second,
		MaxRetries: 3 + 2,
	}
}

func (l Limit3) ClampDepth(n int) int {
	if n < 0 {
		return 0
	}
	if n > l.MaxDepth {
		return l.MaxDepth
	}
	return n
}

func (l Limit3) Extend(base time.Duration) time.Duration {
	if base <= 0 {
		return l.LeaseTTL
	}
	return base
}

func (l Limit3) AllowRetry(attempt int) bool {
	return attempt <= l.MaxRetries
}

func MergeLimit3(a, b Limit3) Limit3 {
	out := a
	if b.MaxDepth > 0 && (a.MaxDepth == 0 || b.MaxDepth < a.MaxDepth) {
		out.MaxDepth = b.MaxDepth
	}
	if b.LeaseTTL > out.LeaseTTL {
		out.LeaseTTL = b.LeaseTTL
	}
	if b.MaxRetries > out.MaxRetries {
		out.MaxRetries = b.MaxRetries
	}
	return out
}
