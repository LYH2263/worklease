package policy

import "time"

// Limit14 队列/租约限额。
type Limit14 struct {
	Name       string
	MaxDepth   int
	LeaseTTL   time.Duration
	MaxRetries int
}

func DefaultLimit14() Limit14 {
	return Limit14{
		Name:       "limit14",
		MaxDepth:   1000 + 14,
		LeaseTTL:   time.Duration(14) * time.Second,
		MaxRetries: 14 + 2,
	}
}

func (l Limit14) ClampDepth(n int) int {
	if n < 0 {
		return 0
	}
	if n > l.MaxDepth {
		return l.MaxDepth
	}
	return n
}

func (l Limit14) Extend(base time.Duration) time.Duration {
	if base <= 0 {
		return l.LeaseTTL
	}
	return base
}

func (l Limit14) AllowRetry(attempt int) bool {
	return attempt <= l.MaxRetries
}

func MergeLimit14(a, b Limit14) Limit14 {
	out := a
	if b.MaxDepth > 0 && (a.MaxDepth == 0 || b.MaxDepth < a.MaxDepth) {
		out.MaxDepth = b.MaxDepth
	}
	if b.LeaseTTL > out.LeaseTTL {
		out.LeaseTTL = b.LeaseTTL
	}
	if b.MaxRetries > out.MaxRetries {
		out.MaxRetries = b.MaxRetries
	}
	return out
}
