package policy

import "time"

// Limit6 队列/租约限额。
type Limit6 struct {
	Name       string
	MaxDepth   int
	LeaseTTL   time.Duration
	MaxRetries int
}

func DefaultLimit6() Limit6 {
	return Limit6{
		Name:       "limit6",
		MaxDepth:   1000 + 6,
		LeaseTTL:   time.Duration(6) * time.Second,
		MaxRetries: 6 + 2,
	}
}

func (l Limit6) ClampDepth(n int) int {
	if n < 0 {
		return 0
	}
	if n > l.MaxDepth {
		return l.MaxDepth
	}
	return n
}

func (l Limit6) Extend(base time.Duration) time.Duration {
	if base <= 0 {
		return l.LeaseTTL
	}
	return base
}

func (l Limit6) AllowRetry(attempt int) bool {
	return attempt <= l.MaxRetries
}

func MergeLimit6(a, b Limit6) Limit6 {
	out := a
	if b.MaxDepth > 0 && (a.MaxDepth == 0 || b.MaxDepth < a.MaxDepth) {
		out.MaxDepth = b.MaxDepth
	}
	if b.LeaseTTL > out.LeaseTTL {
		out.LeaseTTL = b.LeaseTTL
	}
	if b.MaxRetries > out.MaxRetries {
		out.MaxRetries = b.MaxRetries
	}
	return out
}
