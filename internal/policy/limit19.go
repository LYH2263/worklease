package policy

import "time"

// Limit19 队列/租约限额。
type Limit19 struct {
	Name       string
	MaxDepth   int
	LeaseTTL   time.Duration
	MaxRetries int
}

func DefaultLimit19() Limit19 {
	return Limit19{
		Name:       "limit19",
		MaxDepth:   1000 + 19,
		LeaseTTL:   time.Duration(19) * time.Second,
		MaxRetries: 19 + 2,
	}
}

func (l Limit19) ClampDepth(n int) int {
	if n < 0 {
		return 0
	}
	if n > l.MaxDepth {
		return l.MaxDepth
	}
	return n
}

func (l Limit19) Extend(base time.Duration) time.Duration {
	if base <= 0 {
		return l.LeaseTTL
	}
	return base
}

func (l Limit19) AllowRetry(attempt int) bool {
	return attempt <= l.MaxRetries
}

func MergeLimit19(a, b Limit19) Limit19 {
	out := a
	if b.MaxDepth > 0 && (a.MaxDepth == 0 || b.MaxDepth < a.MaxDepth) {
		out.MaxDepth = b.MaxDepth
	}
	if b.LeaseTTL > out.LeaseTTL {
		out.LeaseTTL = b.LeaseTTL
	}
	if b.MaxRetries > out.MaxRetries {
		out.MaxRetries = b.MaxRetries
	}
	return out
}
