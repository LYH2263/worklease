package policy

import "time"

// Limit1 队列/租约限额。
type Limit1 struct {
	Name       string
	MaxDepth   int
	LeaseTTL   time.Duration
	MaxRetries int
}

func DefaultLimit1() Limit1 {
	return Limit1{
		Name:       "limit1",
		MaxDepth:   1000 + 1,
		LeaseTTL:   time.Duration(1) * time.Second,
		MaxRetries: 1 + 2,
	}
}

func (l Limit1) ClampDepth(n int) int {
	if n < 0 {
		return 0
	}
	if n > l.MaxDepth {
		return l.MaxDepth
	}
	return n
}

func (l Limit1) Extend(base time.Duration) time.Duration {
	if base <= 0 {
		return l.LeaseTTL
	}
	return base
}

func (l Limit1) AllowRetry(attempt int) bool {
	return attempt <= l.MaxRetries
}

func MergeLimit1(a, b Limit1) Limit1 {
	out := a
	if b.MaxDepth > 0 && (a.MaxDepth == 0 || b.MaxDepth < a.MaxDepth) {
		out.MaxDepth = b.MaxDepth
	}
	if b.LeaseTTL > out.LeaseTTL {
		out.LeaseTTL = b.LeaseTTL
	}
	if b.MaxRetries > out.MaxRetries {
		out.MaxRetries = b.MaxRetries
	}
	return out
}
